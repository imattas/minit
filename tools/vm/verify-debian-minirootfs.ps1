...
if ($Help) {
    Write-Output "Usage: verify-debian-minirootfs.ps1 -Kernel <bzImage> (-RootfsTar <debian-rootfs.tar[.*]> | -RootfsDir <root>) [-Profile minimal-distro] [-SmokeUnit sshd.service]"
    exit 0
}
...
if ($RootfsTar -and -not (Test-Path -LiteralPath $RootfsTar)) {
    Stop-WithError "RootfsTar must point to a local Debian root filesystem tarball." 2
}
if ($RootfsDir -and -not (Test-Path -LiteralPath $RootfsDir -PathType Container)) {
    Stop-WithError "RootfsDir must point to an extracted Debian root filesystem directory." 2
}

$wsl = Get-Command wsl.exe -ErrorAction SilentlyContinue
if (-not $wsl) {
    Stop-WithError "wsl.exe with bash, tar, cp, find, chmod, and cpio is required for Debian rootfs verification." 3
}
...
$artifactDir = Join-Path $PSScriptRoot "artifacts\debian-minirootfs"
$root = Join-Path $artifactDir "root"
$initramfs = Join-Path $artifactDir "debian-minirootfs.cpio"
...
$scriptPath = Join-Path $artifactDir "build-debian-minirootfs-initramfs.sh"
...
Invoke-Step "build Debian minirootfs initramfs" {
    & $wsl.Source bash $bashScriptPath
}

Invoke-Step "boot Debian rootfs status smoke" {
...
Invoke-Step "boot Debian rootfs list smoke" {
    powershell -NoProfile -ExecutionPolicy Bypass -File tools\vm\run-minit-qemu.ps1 `
        -Kernel $Kernel `
        -Initramfs $initramfs `
        -NormalMode `
...
Invoke-Step "boot Debian rootfs clean shutdown smoke" {
    powershell -NoProfile -ExecutionPolicy Bypass -File tools\vm\run-minit-qemu.ps1 `
        -Kernel $Kernel `
        -Initramfs $initramfs `
        -NormalMode `
...
Write-Host "Debian minirootfs VM verification passed."
