...
Current milestone: VM-proven experimental release candidate with `minitd` as PID 1, cgroups v2 supervision, `minitctl` status/list/start/stop/restart/explain/graph/events/logs/boot-timeline, JSON dependency graph output, target boot, mount/swap units, diagnostic events, hardening proof, release packaging, and QEMU smoke coverage.
...
Release notes and known limitations for the latest experimental milestone are in [docs/releases/v0.3.0-experimental.md](docs/releases/v0.3.0-experimental.md).
